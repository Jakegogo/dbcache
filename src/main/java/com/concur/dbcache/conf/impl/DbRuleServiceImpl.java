package com.concur.dbcache.conf.impl;

import com.concur.dbcache.conf.CfgConstants;
import com.concur.dbcache.conf.DbRuleService;
import com.concur.dbcache.pkey.IdGenerator;
import com.concur.dbcache.pkey.LongGenerator;
import com.concur.dbcache.pkey.ServerEntityIdRule;
import com.concur.dbcache.support.jdbc.ModelInfo;
import com.concur.dbcache.IEntity;
import com.concur.dbcache.dbaccess.DbAccessService;
import com.concur.unity.reflect.GenericsUtils;
...
public class DbRuleServiceImpl implements DbRuleService {
	...
	private String entityPackages = "com/concur/dbcache";
	...
	public void init() {
		...
		//初始化服ID列表
		if (properties.containsKey(CfgConstants.KEY_SERVER_ID_SET)) {
			this.initServerIdSet(properties.getProperty(CfgConstants.KEY_SERVER_ID_SET));
		}

		if (this.serverIdList == null || this.serverIdList.size() < 1) {
			FormattingTuple message = MessageFormatter.format(
					"DbCached [{}] 配置项 '服务器ID标识集合[{}]' 配置错误!", location, CfgConstants.KEY_SERVER_ID_SET);
		...
		try {
			dbPoolSize = Integer.parseInt(properties.getProperty(CfgConstants.KEY_DB_POOL_CAPACITY));
		} catch (Exception ex) {
			logger.error("转换'{}'失败， 使用缺省值", CfgConstants.KEY_DB_POOL_CAPACITY);
		}
		...
		try {
			entityCacheSize = Integer.parseInt(properties.getProperty(CfgConstants.MAX_QUEUE_SIZE_BEFORE_PERSIST));
		} catch (Exception ex) {
			logger.error("转换'{}'失败， 使用缺省值", CfgConstants.MAX_QUEUE_SIZE_BEFORE_PERSIST);
		}
		...
		try {
			delayWaitTimmer = Integer.parseInt(properties.getProperty(CfgConstants.DELAY_WAITTIMMER));
		} catch (Exception ex) {
			logger.error("转换'{}'失败， 使用缺省值", CfgConstants.MAX_QUEUE_SIZE_BEFORE_PERSIST);
		}
	...
	private void initServerIdSet(String serverIdSet) {
		...
		String[] serverIdArray = serverIdSet.trim().split(CfgConstants.SPLIT);
